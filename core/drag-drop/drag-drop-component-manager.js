...
DragDropComponentManager.prototype.findActiveDropZone = function (dragEvent) {
    var pointerPositionX = dragEvent.startPositionX + dragEvent.translateX, pointerPositionY = dragEvent.startPositionY + dragEvent.translateY, dragOverElement = document.elementFromPoint(pointerPositionX, pointerPositionY), AbstractDropZoneComponent = AbstractDropZoneComponentModule.AbstractDropZoneComponent, candidateComponent = null;
    if (dragOverElement) {
        var candidateElement = dragOverElement, foundCandidateComponent = false;
...
DragDropComponentManager.prototype._dispatchEventToComponent = function () {
    if (arguments.length > 0) {
        var dropZoneComponents = this._dropZoneComponentRegistry, dropZoneComponentKeys = Object.keys(dropZoneComponents), handler = arguments[0], dropZoneComponent;
...
DragDropComponentManager.prototype.listenFilesDragDrop = function () {
    var self = this, targetCounter = 0, handleFileDragEnter = function (event) {
