    ...
    mapSelectorToRawDataSelector: {
        value: function(montageDataSelector) {
            return {
                ...
                limit: montageDataSelector.__limit
            }
        }
    },
    ...
    _getMiddlewareCriteriaFromObject: {
        value: function(object) {
            ...
            for (var i = 0, length = keys.length; i < length; i++) {
                key = keys[i];
                value = object[key];
                if (typeof value === 'object' && !value._isCustom) {
                    ...
                    if (Array.isArray(value)) {
                        for (var i = 0; i < value.length; i++) {
                            criteria.push([key, value[i].operator, value[i].value]);
                        }
                    } else {
                        criteria.push([key, value.operator, value.value]);
                    }
                } else {
    ...
    fetchRawData: {
        value: function (stream) {
            ...
            var self = this,
                ...
                criteria = stream.selector.criteria.length > 0 || stream.selector.limit ? [stream.selector.criteria] : stream.selector.criteria,
            ...
            if (stream.selector.limit) {
                criteria.push({limit: stream.selector.limit});
            }
